...
class LinkButton extends StatefulWidget {
  ...
  LinkButton({Key key, @required this.icon, @required this.url}) : super(key: key);
...
class LinkButtonState extends State<LinkButton> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      child: Container(
          child: IconButton(icon:widget.icon,
                        color: _color,
                        iconSize: 40.0,
                        onPressed: () {_launchURL(widget.url);},).showCursorOnHover.moveUpOnHover,
        ),
      onTap: () {
        //Not needed.
      },
